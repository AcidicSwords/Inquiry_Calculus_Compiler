...
`INQUIRY_SPINE_CONTRACT.json` is the compact derived machine contract for the one model-facing
recurrence defined in the construction specification. It retains line-addressed ordinary question
forms from the Coding and final Reciprocal Why corpus regions, relation-triggered generators,
lifecycle evidence requirements, method contracts, fold/reopening requirements, and non-collapse
...
retains their historical ancestry. `RESIDUAL_OBLIGATIONS.json` remains a stable obligation seed,
not another cursor or procedure.
...
`PHASE_B_TRAVERSAL_LEARNING.md` separates that path-fold route from probe-basis
extension. A generated probe requires positive old-basis nondistinction, candidate
separation, and protected-consequence separation; it remains inert until independent
later-use evidence supports admission.

`PHASE_B_QUESTION_PATTERN_LEARNING.md` keeps the complete realized occurrence chain
primary while treating operator and state/boundary views as lossy lookup projections.
Aggregation requires explicit protection and applicability; continuation selection
requires exact chain recovery.

`PHASE_B_MEMORY_RECOVERY.md` keeps use, reconstruction, evaluation, and reacquisition
