...
\item An admitted attribute enlarges the constructible question space.
...
A determination may admit multiple negation uses.  They remain a tagged family: the relation use that licensed an exterior remains part of the reciprocal occurrence because different uses can return different source fields.  Inquiry partially binds one such relation to the source and positively determines an exterior filling.  Pure return is the reverse section of that same relation.  The return fiber recovers a protected source relation exactly when every source still possible through that fiber agrees on its protected signature.  Multiple negation uses accumulate reusable information through the product of their return signatures, not through an untagged semantic union.
