...
| P6-EFFECT-JOURNAL-001 durable external-effect preparation precedes completion, is idempotent only for exact data, allows one unresolved single-writer effect, and survives restart as unknown/pending | PASS | `ic_store::tests::external_effect_preparation_survives_restart_and_completes_as_one_raw_event` |
...
| Actuality, ledger/domain ordering, resolution, and replay | PARTIAL | Phase 6: opaque immutable raw-return, canonical event identity, append-only parent-linked ledger checks, typed request-before-dispatch, pending/unknown recovery, atomic raw/event completion, and file-backed restart/revalidation pass. Phase 7: typed resolution-route identity/composition, canonical complete-candidate identity/checking, finite direct single-port decoded/undefined/unknown event results, exact supported-answer admission/binding, and finite event-to-runtime resumption pass; typed semantic attempts, dispatch, opaque-contract validation, state-transition replay, multi-port answer carriers, general route execution, relation evaluation, and full resolution remain pending |
...
| Surface/backend/raw-return compilation boundaries | PARTIAL | Phase 10: canonical compiled probe-operator, recurrent probe-contract, surface-plan, backend-request, and method-contract identities are distinct from runtime control flow and raw return; exact operator/plan/request checking and typed durable preparation pass. Method admission/runnable/usable classification, semantic resolution, contract comparability/bridges, actual rendering, dispatch, and provider execution remain pending |
