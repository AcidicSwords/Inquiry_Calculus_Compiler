...
| Surface/backend/raw-return compilation boundaries | PARTIAL | Phase 10: canonical compiled probe-operator, recurrent probe-contract, surface-plan, backend-request, and method-contract identities are distinct from runtime control flow and raw return; exact operator/plan/request checking, typed durable preparation, injected mock-provider execution, and a narrow OpenAI Responses transport contract pass offline. HTTP success and non-success responses remain exact post-dispatch actuality through a versioned status/body frame. Successful live execution, Responses semantic decoding, method admission/runnable/usable classification, contract comparability/bridges, and general rendering remain pending; the current live credential constraint is `F-0001`. |
...
| PROVIDER-001 live successful actualization | PENDING | The opt-in fixture reached the official Responses endpoint and committed HTTP 401 before interpretation. `F-0001` records the credential constraint; no successful response, decoded completion set, supported binding, resumption, or cold replay is claimed. |
