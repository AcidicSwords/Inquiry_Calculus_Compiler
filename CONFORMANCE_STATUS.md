...
| PROVIDER-001E live typed decode-to-replay slice | PENDING | Requires one fresh `qwen3.5:9b` dispatch to create the raw event and only then construct the entire `PROVIDER-001D` semantic/replay chain, with no pre-existing candidate, support, decoder, or observation artifacts. |
