...
const output = path.join(root, "formal-successor", "PHASE_B_MINIMAL_LOGICAL_BASIS_SURFACE.json");
...
function surface() {
  const schema = read("formal-successor/PHASE_B_MINIMAL_LOGICAL_BASIS_SCHEMA.json");
  ...
  return {
    schema: 1,
    status: "generated_phase_b_minimal_logical_basis_surface_not_successor_semantics",
    generated_from: {
      schema_sha256: digest(schema.bytes),
      tex_classification_sha256: digest(classification.bytes),
      basis_module_sha256: digest(fs.readFileSync(path.join(root, schema.value.inputs.basis_module))),
    },
    records,
    coverage: {
...
try {
  ...
  if (process.argv[2] === "generate") {
    ...
    process.stdout.write("generated Phase B minimal logical basis surface\n");
  } else if (process.argv[2] === "check" && fs.readFileSync(output, "utf8") === expected) {
    process.stdout.write("Phase B minimal logical basis surface regenerates exactly\n");
  } else {
    throw new Error("minimal logical basis surface does not exactly regenerate");
  }
} catch (error) {
  process.stderr.write(`Phase B minimal logical basis: ${error.message}\n`);
  process.exit(1);
}
