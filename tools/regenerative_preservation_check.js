...
const moduleName = "InquiryCalculus.Legacy.V20.RegenerativePreservation";
const modulePath = "formal/InquiryCalculus/Legacy/V20/RegenerativePreservation.lean";
const texDigest = "1f548e0fa3e8374a01b6268e813cedcc757b26ed460adb5b82fe8ba60ca1dd89";
const sources = new Map([
  ["PRED-TEX-PROSE-5151F3681D12EB2C", [4669, 4669, "Ambiguous"]],
  ["PRED-TEX-DISPLAY-A83800F1D58DDFA9", [4670, 4674, "Ambiguous"]],
  ["PRED-TEX-PROSE-00FA8313032DBA37", [4675, 4675, "Unproved"]]
]);
...
function verifySources(tex, classification) {
  ...
  for (const [id, [start, end, status]] of sources) {
    ...
    assert.match(record.destination, new RegExp(`/Obligations/regenerative-preservation/${id}$`, "u"));
  }
}
...
function main() {
  ...
  const doc = read("formal-successor/PHASE_B_REGENERATIVE_PRESERVATION.md").toString();
  verifySources(tex, classification);
  for (const token of ["RegenerationContext", "RegenerativelySufficient", "witnessContext",
    "wrongSourceContext", "witnessContextIsRegenerativelySufficient", "wrongSourceContextFails",
    "currentConsequenceDoesNotImplyRegeneration"]) assert.match(lean, new RegExp(`\\b${token}\\b`, "u"));
  assert.match(doc, /three exact `LegacyObligation` records at v2\.0\s+lines 4669–4675/u);
  assert.doesNotMatch(lean, /\b(?:sorry|admit|axiom|unsafe|native_decide|Classical\.choose)\b/u);
  assert.match(read("formal/InquiryCalculus.lean").toString(), /^import InquiryCalculus\.Legacy\.V20\.RegenerativePreservation\r?$/mu);
  console.log(`PASS exact regenerative-preservation sources and wrong-source contrast; module sha256 ${hash(lean)}`);
  if (!process.argv.includes("--compile")) return;
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "ic-regenerative-preservation-"));
  ...
  const audits = ["Countermodel.witnessContextIsRegenerativelySufficient", "Countermodel.wrongSourceContextFails", "Countermodel.currentConsequenceDoesNotImplyRegeneration"];
  ...
  const ablations = [["future-use", "  protectedFutureUse : Source → Prop", "  protectedFutureUse : True"], ["regeneration-witness", "  regenerates : Target → Source → Prop", "  regenerates : True"], ["regenerative-law", "def RegenerativelySufficient", "def RegenerativelySufficientRemoved"], ["witness-contrast", "def witnessContext", "def witnessContextRemoved"], ["wrong-source-foil", "def wrongSourceContext", "def wrongSourceContextRemoved"]];
  ...
  console.log(`PASS five regeneration ablations and ${audits.length} axiom-free proof audits`);
}
main();
