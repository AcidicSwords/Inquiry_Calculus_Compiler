...
Construct a decisive source-bound joint-candidate incidence foil, then contrast it with departure,
negation, crossing, completed-occurrence, selected-answer, solver, program, semantic-evaluation, and
omission claims. Contract only after exact source regeneration and retained obligations distinguish the
smallest incidence boundary.

## Prohibitions at this boundary

- Do not treat candidate boundary incidence as a departure witness, negation relation, crossing, completed occurrence, selected answer, total solver, program, or successor semantics.
- Do not use a source incidence record alone as lawful departure, negation, occurrence, answer-validity, or solver evidence.
...
On pass, retain the smallest source-regenerative candidate-boundary-incidence boundary, explicit
source obligations, and independent breakers, then select the next residual. On failure, preserve the
return and reopen the earliest candidate pair, boundary point, projection, departure, negation,
crossing, occurrence, or source boundary relation.
